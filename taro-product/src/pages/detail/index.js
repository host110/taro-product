...
export default class Detail extends Component {
  ...
  async getGoodsInfo(goodsId) {
    ...
    if (res.errno == 0) {
      if (res.data != null) {
        //res.data = String(res.data).split('\n');
      } else {
        res.data = [];
      }
        ...
        <View className='container'>
          ...
          <View className='info-size'>
            { specificationsList && specificationsList.length > 0 && specificationsList.map((spe, speIndex) => {
              console.log(specificationsList,'1111111111111');
              return (
                <View key={speIndex}>
          ...
          { /* 品牌介绍 */}
          {detail.subtitle && detail.subtitle != null && (
            <View className='goods-info'>
              <View className='chapter-head'>品牌介绍</View>
              <View className='introduce'>
                <View className='b'>{detail.subtitle}</View>
